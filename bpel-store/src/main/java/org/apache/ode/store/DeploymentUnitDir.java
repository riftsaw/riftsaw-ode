...
class DeploymentUnitDir {
    ...
    DeploymentUnitDir(File dir) {
        ...
        if (!_descriptorFile.exists()) {
            _descriptorFile = new File(_duDirectory, "bpel-deploy.xml");
        }
    ...
    public void setDeployDir(File file) {    	
    	...
    	// Re-scan due to new deployment unit location
    	scan();
    }
    ...
    public DeployDocument getDeploymentDescriptor() {
        if (_dd == null) {
            ...
            if (!ddLocation.exists()) {
            	ddLocation = new File(_duDirectory, "bpel-deploy.xml");
            }
